...
use crate::parsing::{ParsingError, Token};
...
pub fn test_parser<T: Debug + Display + Eq + Clone>(
    ...
    for ParsingTest { test_name, tokens, expected, next_token } in tests {
        ...
        assert!(got == *expected, "{}", assert_failed_message(test_name, tokens, &got, expected));
        assert!(iter_next == *next_token, "{}", assert_message_wrong_next_token(test_name, tokens, &iter_next, next_token));
    }
}

fn format_result<T: Display, E: Display>(result: &Result<T, E>) -> String {
    match result {
        Ok(ok) => format!("{}", ok),
        Err(err) => format!("{}", err)
    }
...
fn assert_failed_message<T: Display, E:Display>(
    test_name: &str,
    tokens: &[Token],
    got: &Result<T, E>,
    expected: &Result<T, E>
